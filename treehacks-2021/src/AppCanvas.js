...
const FaceFollower = (props) => {
  ...
  const faceURL = './test.obj';
  const [face, setFace] = useState();
   
  useMemo(() => new OBJLoader().load(faceURL, setFace), [faceURL]);
    
  const boxGeo = <boxBufferGeometry args={[1, 1, 1]} />;
  const faceTest = <primitive object={face} />;
  console.log(boxGeo);
  if (face) {
      console.log(face);
  }
  ...
  return (
    <object3D ref = {objRef} dispose={null}>
      {face ? <object3D object={face.children[0]} /> : null }
    </object3D>
  )
}
