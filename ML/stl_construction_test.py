...
def construct_fitter(input_file):
    ...
    def get_simplified_slice(sobj_mesh):
        nice_slice = sobj_mesh.section(plane_origin=sobj_mesh.centroid + np.array([0, 0, 30]), plane_normal=[0.55,0,1])
    ...
    def filter_vertices(pnts, close_loop):
        ...
        for i in range(2, len(pnts)):
            ...
            cmd = subprocess.run(["python", "test_point.py", "filter_vertices.txt"], capture_output=True)
