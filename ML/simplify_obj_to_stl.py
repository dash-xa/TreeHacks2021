...
def simplify_obj_for_ar(input_filename, output_filename=None):
    if output_filename is None:
        output_filename = input_filename
    ...
    print("new number of faces:", len(sobj_mesh.faces))
    print("original scale:", sobj_mesh.scale)
    sobj_mesh.apply_scale(100/sobj_mesh.scale)
    print("new scale:", sobj_mesh.scale)
    sobj_mesh.vertices -= (sobj_mesh.vertices.max(axis=0) + sobj_mesh.vertices.min(axis=0)) / 2
    # sobj_mesh.vertices -= sobj_mesh.centroid
    trimesh.repair.fix_inversion(sobj_mesh)
...
if __name__ == "__main__":
    ...
    simplify_obj_for_ar('3d_files/cdot_test_0.obj', '3d_files/cdot_test_dss_inverted_ar.obj')
